...
from repatch_data import extract_unique_sceneids, get_patches_for_sceneid, stitch_scene, \
    rotate_and_crop, fit_image
...
def extract_patches(tiff_file, patch_size, output_dir, scene_id, gt_dir):
    ...
    with Image.open(tiff_file) as img:
        ...
        for i in range(num_patches_y + 1):  # Include an extra iteration for the extra row
            for j in range(num_patches_x + 1):  # Include an extra iteration for the extra column
...
def download_landsat_band(scene_id, band_name, output_dir):
    ...
    # Construct the command to download the specific band
    command = [
        'pylandsat', 'download',
        '--files', band_name,
        scene_id
